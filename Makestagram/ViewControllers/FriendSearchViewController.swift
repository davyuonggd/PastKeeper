...
class FriendSearchViewController: UIViewController {
    ...
    */
    var followingUsers: [PFUser]? {
        didSet {
            //the list of following users may be fetched after the tableView has displayed cells. In this case, we reload the data to reflect "following" status
    ...
    //this view can be in 2 different states
    enum State {
        case DefaultMode
        case SearchMode
    }
    ...
    */
    ...
    //whenever the state changes, perform 1 of the 2 queries and update the list
    var state: State = .DefaultMode {
        didSet {
            ...
            case .SearchMode:
                let searchText = searchBar?.text ?? "" //The nil coalescing operator (a ?? b) unwraps an optional a if it contains a value, or returns a default value b if a is nil. The expression a is always of an optional type. The expression b must match the type that is stored inside a.
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        ParseHelper.getFollowingUsersForUser(PFUser.currentUser()!, completionBlock: { (results: [AnyObject]?, error: NSError?) -> Void in
            
            let relations = results as? [PFObject] ?? []
            //use map to extract the User from a Follow object
            self.followingUsers = relations.map { $0[ParseHelper.ParseFollowToUser] as! PFUser }
...
extension FriendSearchViewController: FriendSearchTableViewCellDelegate {
    ...
    func cell(cell: FriendSearchTableViewCell, didSelectUnfollowUser user: PFUser) {
        if var followingUsers = self.followingUsers {
